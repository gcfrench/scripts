...
map works best with the list of lists structure where it can extract an element from each list

```{r}
map(list_of_lists, "c")
```

In the nested list structure map functions will iterate through the first list element in the list. In the nested list example, first drill down to the 3rd level using subsetting and then map will return NA for the first three elements as they are not lists, and "4c" from the fourth list element.
...
```{r}
map(list(nested_list), extract, "c")
```
